...
import { Input } from './components/ui/input'; // Added for the chat input
import './App.css';
...
const App = () => {
  ...
  const [chats, setChats] = useState({}); // Stores all chat histories, e.g., { userId: [messages] }
  const [activeChatUser, setActiveChatUser] = useState(null); // The user object of the person we are chatting with
  ...
  // NEW: Scroll to the bottom of the chat window when new messages arrive
  useEffect(() => {
  ...
  // NEW: Function to process and store a new private message
  const handlePrivateMessage = (message) => {
    // Message from others will have a different from_user_id
    // The copy we receive of our own message will have our user_id
    ...
    setChats(prevChats => ({
      ...prevChats,
      [partnerId]: [...(prevChats[partnerId] || []), newMessage]
    }));

    // Notify user if they are not already chatting with the sender
  ...
  // NEW: Function to send a private chat message
  const sendPrivateMessage = () => {
    ...
    if (websocketRef.current && websocketRef.current.readyState === WebSocket.OPEN) {
        websocketRef.current.send(JSON.stringify({
            ...
            to_user_id: activeChatUser.user_id,
                    ...
                    <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
                        {(chats[activeChatUser.user_id] || []).map((msg, index) => (
                            <div key={index} className={`flex items-end gap-2 ${msg.sender === myCharacter ? 'justify-end' : 'justify-start'}`}>
                                {msg.sender !== myCharacter && <Badge variant="secondary" className="bg-slate-600 text-gray-200">{msg.sender.charAt(0)}</Badge>}
                                ...
                                {msg.sender === myCharacter && <Badge variant="secondary" className="bg-blue-600 text-white">{myCharacter.charAt(0)}</Badge>}
                            </div>
                        ))}
